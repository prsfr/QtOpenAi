...
class QTOPENAI_CLIENT_EXPORT VectorStoreReply : public TypedReply<Core::VectorStore>
{
    Q_OBJECT
public:
    Core::VectorStore store() const { return value(); }
...
private:
    ...
    void emitFinished(const Core::VectorStore &store) override { Q_EMIT finished(store); }
};
